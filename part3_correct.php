...
$params = array();
    ...
    {
        array_push($params,filter_var($_POST['c_name'],FILTER_SANITIZE_STRING));
    ...
    //Category field
    if (!empty($_POST['c_category_id']))
    {
        if (count($params) >0 )
        {
            array_push($params,filter_var($_POST['c_category_id'],FILTER_SANITIZE_STRING));
    ...
    //Price minimum field
    if (!empty($_POST['c_pricemin']))
    {
        if (count($params) >0 )
        {
            array_push($params,filter_var($_POST['c_pricemin'],FILTER_SANITIZE_STRING));
    ...
    //Price max field
    if (!empty($_POST['c_pricemax']))
    {
        if (count($params) >0 )
        {
            array_push($params,filter_var($_POST['c_pricemax'],FILTER_SANITIZE_STRING));
            $query_condition.= "AND price <= $".count($params)." ";
            echo "Query now = ".$query_condition;
            echo "<br>";
        }
...
////////////////////// SEARCH FIELD EMPTY
    ...
    //Search for anywhere
    if (!empty($_POST['c_search_input']) and empty($_POST['c_search_field']))
    {
        //Match any words
        if ($_POST['c_radio_match'] == "any")
            {
                if (count($params) >0 )
                {
                    array_push($params,filter_var($_POST['c_search_input'],FILTER_SANITIZE_STRING));
    ...
    if (!empty($_POST['c_search_input']) and !empty($_POST['c_search_field']))
    {
        ...
        if ($_POST['c_radio_match'] == "any")
        {
            if (count($params) >0 )
            {
                ...
                array_push($params,filter_var($_POST['c_search_input'],FILTER_SANITIZE_STRING));
                $query_condition.= "AND ".$_POST['c_search_field']. " like $".count($params)." ";
                echo "Query now = ".$query_condition;
                echo "<br>";
            }
            else {
                echo "c_search_input is set = ".$_POST['c_search_input'] ;
                echo "<br>";
                array_push($params,filter_var($_POST['c_search_input'],FILTER_SANITIZE_STRING));
                $query_condition.= "WHERE ".$_POST['c_search_field']. " like $".count($params)." ";
                echo "Query now = ".$query_condition;
                echo "<br>";
            }
        }
        ...
        if ($_POST['c_radio_match'] == "all")
        {
            if (count($params) >0 )
            {
                ...
                array_push($params,filter_var($_POST['c_search_input'],FILTER_SANITIZE_STRING));
                $query_condition.= "AND ".$_POST['c_search_field']." = $".count($params)." ";
                echo "Query now = ".$query_condition;
                echo "<br>";
            }
            else {
                ...
                array_push($params,filter_var($_POST['c_search_input'],FILTER_SANITIZE_STRING));
                $query_condition.= "WHERE ".$_POST['c_search_field']." = $".count($params)." ";
                echo "Query now = ".$query_condition;
                echo "<br>";
            }
        }
...
///
    if (!empty($_POST['c_sp_d']))
    {
        ...
        {
            if (!empty($_POST["c_sp_date_range"]) && $_POST['c_sp_date_range'] != -1)
            {
                if (count($params) > 0)
                {
                    array_push($params, filter_var("-".$_POST['c_sp_date_range'], FILTER_SANITIZE_STRING));
        ...
        {
            if (!empty($_POST["c_sp_start_month"]) && !empty($_POST["c_sp_start_day"]) && !empty($_POST["c_sp_start_year"]) && !empty($_POST["c_sp_end_month"]) && !empty($_POST["c_sp_end_day"]) && !empty($_POST["c_sp_end_year"])) {
                $s_date = $_POST['c_sp_start_year'] . "-" . $_POST["c_sp_start_month"] . "-" . $_POST["c_sp_start_day"];
                $e_date = $_POST["c_sp_end_year"] . "-" . $_POST["c_sp_end_month"] . "-" . $_POST["c_sp_end_day"];
                if (count($params) > 0)
                {
                    array_push($params, filter_var($s_date, FILTER_SANITIZE_STRING));
                    array_push($params, filter_var($e_date, FILTER_SANITIZE_STRING));
            ...
            {
                echo "You should write the fields properly";
            }
        }
    }
    ...
    if (!empty($query_condition))
        {
            ...
            echo "<br>";
            $result = pg_prepare($db, "my_query", 'SELECT * FROM books '.$query_condition);
            // Execute the prepared query
            $result = pg_execute($db, "my_query", $params);
